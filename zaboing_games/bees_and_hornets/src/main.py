...
debug = False
# Entities
entities = [Rectangle(0, 0, 10, 10), Rectangle(11, 0, 10, 10), Rectangle(22, 0, 10, 10)]
...
init_window(300, 300, "raylib beees and hornets")
set_target_fps(60)
...
# Player
player = Entity(
    10, 2, entity_atlas, entities[0], Rectangle(138, 138, 25, 25), 0, WHITE, None
)
mineral_counter = 0
while not window_should_close():
    ...
    bee = Entity(
        10,
        random.randint(10, 15) / 10,
        entity_atlas,
        entities[1],
        Rectangle(200, 200, 15, 15),
        0,
        WHITE,
        Vector2(0, 0),
    )
    mineral = Entity(
        ...
        entity_atlas,
        entities[2],
        Rectangle(random.randint(-1000, 1000), random.randint(-1000, 1000), 10, 10),
        0,
        WHITE,
        Vector2(0, 0),
    )
    ...
    for i in bees:
        draw_entity_sprite(i)
        i.extra = get_direction(
            Vector2(beedest.x + camera.target.x, beedest.y + camera.target.y),
            Vector2(i.rectangle.x, i.rectangle.y),
        )
        if i.extra.x < 0:
            i.source.width = -10
        else:
    ...
    if debug:
        draw_text(
            f"x:{player.rectangle.x} y:{player.rectangle.y}\nbees:{len(bees)}\nminerals:{mineral_counter}",
            0,
            0,
            10,
            GREEN,
