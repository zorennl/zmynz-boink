...
class particle:
    def __init__(self, pos: Vector2, radius, angle, speed, life: float, color):
        ...
        self.velocity = Vector2(
            self.speed * cos(self.radians), -self.speed * sin(self.radians)
        )
    ...
    def draw(dt, particlelist):
        for i in range(len(particlelist)):
            ...
            if particlelist[i].life > 0:
                ...
                draw_circle_v(
                    particlelist[i].pos, particlelist[i].radius, particlelist[i].color
                )

                if particlelist[i].life == 0:
                    particlelist.pop(i)
                if particlelist[i].angle < 0:
                    particlelist[i].angle += 1
                elif particlelist[i].angle > 0:
                    particlelist[i].angle -= 1
